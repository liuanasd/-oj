...
URL="http://localhost:8080/judge/1"  # 修正为正确的POST接口路径
...
REQUEST_BODY="code=int main(){return 0;}"
...
function send_request() {
    start_time=$(date +%s.%N)
    response=$(curl -s -w "%{http_code} %{time_total}" -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "$REQUEST_BODY" "$URL")
    end_time=$(date +%s.%N)
...
function concurrent_thread() {
    # 使用简单的for循环来控制测试次数
    local iterations=$((DURATION * 10))  # 0.1秒间隔，10次/秒
